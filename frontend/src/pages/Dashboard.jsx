...
const Dashboard = () => {
  ...
  const availableItems = inventory.filter(item => {
    // An item is available if it is not assigned in any active assignment
    return !assignments.some(a => a.itemId._id === item._id && a.status === 'assigned');
  }).length;
  const activeAssignments = assignments.filter(a => a.status === 'assigned').length;

  // Recent assignments (sorted by assignedDate desc)
            ...
            recentAssignments.map((a) => (
              <tr key={a._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${a.status === 'assigned' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                    {a.status}
                  </span>
                </td>
              </tr>
